...
URL libUrl = new URL("jar:" + warLocation.toURI().toURL() + "!/" + "WEB-INF/jetty-server/"
           ...
           // ).loadClass("tested.robots.Ahead");
                ...
                System.err.println("Using ClassLoader: oo  " + clWar);
                System.err.println("Usi:1= ");
                Thread.currentThread().setContextClassLoader(clWar);
                System.err.println("Usi: 2 " + warLocation.toString());

                File warFile = new File(warLocation.toURI());
                System.err.println("Usi: " + warFile.toString());

                System.setProperty("com.hopecee.tublex.jetty.main.LOCATION", warFile.toPath().toRealPath().toString());

                Class<?> mainClass = Class.forName("jetty.main.ServerMain", false, clWar);
